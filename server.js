...
async function startBaileys() {
  // שימוש בנתיב יחסי פשוט ל-Railway
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      // הפעלה בטוחה של ה-Scheduler
      try {
        console.log('📅 מנסה להפעיל את ה-Scheduler...');
        require('./schedule'); 
      } catch (e) {
        console.error('❌ שגיאה בטעינת ה-Schedule:', e.message);
      }
    }
    if (connection === 'close') {
